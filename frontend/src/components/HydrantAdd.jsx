...
// Complete hydrant creation form with map picker and validation
// Optimized for field data entry and municipal standards
...
// Map picker component (simplified for demo)
...
export default function HydrantAdd() {
  ...
  const validateForm = () => {
    ...
    // Watermain size validation
    if (hydrantData.watermain_size_mm < 100 || hydrantData.watermain_size_mm > 600) {
      newErrors.watermain_size_mm = 'Size must be 100-600mm';
    }
    
    // Static pressure validation (if provided)
    if (hydrantData.static_pressure_psi && 
        (hydrantData.static_pressure_psi < 20 || hydrantData.static_pressure_psi > 150)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        // Success - redirect to map or hydrant details
        navigate('/map', { 
          state: { 
  ...
  const generateHydrantNumber = () => {
    // Auto-generate based on location/municipality
    const prefix = 'MLT'; // Milton prefix
      ...
      <Grid container spacing={3}>
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                <Business className="me-2" />Identification
              </Typography>
              ...
              <Stack spacing={3}>
                <Stack direction="row" spacing={1}>
                  <TextField
                    ...
                    placeholder="MLT-2025-001"
                    sx={{ flex: 1 }}
                  />
                  <Button 
                    variant="outlined" 
                    onClick={generateHydrantNumber}
                    sx={{ minWidth: 100 }}
                  >
                    Generate
                  </Button>
                ...
                <FormControl fullWidth error={!!errors.manufacturer}>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      updateField('model', ''); // Reset model when manufacturer changes
                    }}
                    label="Manufacturer *"
                  >
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                <LocationOn className="me-2" />Location
              </Typography>
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                <Build className="me-2" />Specifications
              </Typography>
