...
export default function MobileInspectionMUI() {
  ...
  const submitInspection = async () => {
    ...
    try {
      ...
      // Mock API call - in production this would submit to your maintenance API
      ...
      <CardContent>
        ...
        <Stack spacing={3}>
          ...
          <TextField
            ...
            InputProps={{ startAdornment: <Thermostat /> }}
            fullWidth
          />
      ...
      <CardContent>
        ...
        <Stack spacing={3}>
            ...
            <Grid container spacing={1}>
              {['EXCELLENT', 'GOOD', 'FAIR', 'POOR', 'CRITICAL'].map(condition => (
                <Grid item xs={12} key={condition}>
                  <Button
                    variant={inspectionData.paint_condition === condition ? 'contained' : 'outlined'}
          ...
          <div>
            ...
            <Grid container spacing={1}>
              {['EXCELLENT', 'GOOD', 'FAIR', 'POOR', 'CRITICAL'].map(condition => (
                <Grid item xs={12} key={condition}>
                  <Button
                    variant={inspectionData.body_condition === condition ? 'contained' : 'outlined'}
                    color={getConditionColor(condition)}
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
              <Button
                variant={inspectionData.chains_present ? 'contained' : 'outlined'}
          ...
          <Button
            variant={inspectionData.clearance_adequate ? 'contained' : 'outlined'}
          ...
          <Button
            variant={inspectionData.immediate_action_required ? 'contained' : 'outlined'}
            color={inspectionData.immediate_action_required ? 'error' : 'success'}
            onClick={() => updateField('immediate_action_required', !inspectionData.immediate_action_required)}
            fullWidth
            sx={{ py: 2 }}
          >
            {inspectionData.immediate_action_required ? 
              '⚠️ SAFETY HAZARD IDENTIFIED' : 
              '✓ No Safety Concerns'}
          </Button>

          {inspectionData.immediate_action_required && (
            <TextField
              label="Describe Safety Hazards"
              multiline
              rows={3}
              value={inspectionData.safety_hazards}
              onChange={(e) => updateField('safety_hazards', e.target.value)}
              fullWidth
              required
            />
          )}
      ...
      <CardContent>
        ...
        <Stack spacing={3}>
            ...
            <Grid container spacing={1}>
              {[
                ...
                <Grid item xs={12} key={option.value}>
                  <Button
                    variant={inspectionData.valve_operation === option.value ? 'contained' : 'outlined'}
                    color={option.value === 'SMOOTH' ? 'success' : 
                           option.value === 'STIFF' ? 'warning' : 'error'}
                    onClick={() => updateField('valve_operation', option.value)}
                    fullWidth
                    sx={{ py: 1.5, textAlign: 'left' }}
                  >
          ...
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <TextField
                ...
                type="number"
                value={inspectionData.valve_turns_to_close}
                onChange={(e) => updateField('valve_turns_to_close', e.target.value)}
                placeholder="Count turns"
                fullWidth
              />
            </Grid>
            <Grid item xs={6}>
          ...
          <Grid container spacing={1}>
            <Grid item xs={12}>
              <Button
                variant={inspectionData.valve_leak_detected ? 'contained' : 'outlined'}
                color={inspectionData.valve_leak_detected ? 'error' : 'success'}
                onClick={() => updateField('valve_leak_detected', !inspectionData.valve_leak_detected)}
                fullWidth
                sx={{ py: 2 }}
              >
                {inspectionData.valve_leak_detected ? 
                  '⚠️ Water Leak Detected' : 
                  '✓ No Leaks Detected'}
              </Button>
            </Grid>
            <Grid item xs={12}>
              <Button
                variant={inspectionData.valve_exercised ? 'contained' : 'outlined'}
                color={inspectionData.valve_exercised ? 'success' : 'warning'}
                onClick={() => updateField('valve_exercised', !inspectionData.valve_exercised)}
                fullWidth
                sx={{ py: 2 }}
              >
                {inspectionData.valve_exercised ? 
                  '✓ Valve Exercised (Full Cycle)' : 
                  '⚠️ Valve Not Exercised'}
              </Button>
            </Grid>
          </Grid>
        </Stack>
    ...
    <Card sx={{ mb: 2 }}>
      <CardContent>
        ...
        <Stack spacing={3}>
          <div>
            ...
            <Grid container spacing={1}>
              {['EXCELLENT', 'GOOD', 'FAIR', 'POOR', 'CRITICAL'].map(condition => (
                <Grid item xs={12} key={condition}>
                  <Button
                    variant={inspectionData.overall_condition === condition ? 'contained' : 'outlined'}
                    color={getConditionColor(condition)}
                    onClick={() => updateField('overall_condition', condition)}
                    fullWidth
                    sx={{ py: 1.5 }}
                  >
                    {condition}
          ...
          <Button
            variant={inspectionData.repair_needed ? 'contained' : 'outlined'}
            color={inspectionData.repair_needed ? 'warning' : 'success'}
            onClick={() => updateField('repair_needed', !inspectionData.repair_needed)}
            fullWidth
            sx={{ py: 2 }}
          >
            {inspectionData.repair_needed ? 
              '⚠️ Repair/Maintenance Needed' : 
              '✓ No Repairs Needed'}
          </Button>
          ...
          <Button 
            ...
            sx={{ py: 2 }}
          >
            Take Final Photos ({photos.length} total)
          </Button>
        </Stack>
      </CardContent>
    </Card>
    ...
    <Box sx={{ maxWidth: '100%', p: 2, pb: 10 }}>
      ...
      >
        <Stack direction="row" spacing={1}>
          ...
          <Button 
            ...
            sx={{ flex: 2, py: 1.5 }}
          >
            {activeStep === steps.length - 1 ? 'Review & Submit' : 'Next Step'}
          </Button>
        </Stack>
      </Paper>
      ...
      {(inspectionData.immediate_action_required || 
        ...
        <Alert severity="error" sx={{ mt: 2 }}>
          ...
          <br />
          This inspection will generate an immediate work order.
        </Alert>
      )}
      ...
      <Dialog open={showSummary} maxWidth="md" fullWidth>
        <DialogTitle>
          <Stack direction="row" alignItems="center" spacing={1}>
            <Assignment />
            <Typography variant="h5">Inspection Summary</Typography>
          </Stack>
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <Stack spacing={1}>
                <Stack direction="row" justifyContent="space-between">
                  ...
                  <Chip label={inspectionData.paint_condition} color={getConditionColor(inspectionData.paint_condition)} size="small" />
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography>Body:</Typography>
                  <Chip label={inspectionData.body_condition} color={getConditionColor(inspectionData.body_condition)} size="small" />
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography>Valve:</Typography>
                  <Chip label={inspectionData.valve_operation} color={inspectionData.valve_operation === 'SMOOTH' ? 'success' : 'error'} size="small" />
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography>Pressure:</Typography>
                  <Typography sx={{ fontWeight: 600 }}>{inspectionData.static_pressure_psi} PSI</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                  <Typography>Overall:</Typography>
                  <Chip label={inspectionData.overall_condition} color={getConditionColor(inspectionData.overall_condition)} size="small" />
                </Stack>
              </Stack>
            </Grid>
          </Grid>
          ...
          {inspectionData.inspector_notes && (
            <Box sx={{ mt: 2 }}>
              ...
              <Paper sx={{ p: 2, bgcolor: 'background.default' }}>
                <Typography>{inspectionData.inspector_notes}</Typography>
              </Paper>
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setShowSummary(false)} disabled={loading}>
            Back to Edit
          </Button>
          <Button 
            variant="contained" 
